...
HOME = '/Users/ET/code/test_pingBot2/'  # os.environ['HOME']
...
if __name__=='__main__':
    newadd = []
    backup = []
    newadd, newback = ping('btcservices.csv',newadd,7)
    backup += newback
    try:
        fh = open('/Users/ET/code/test_pingBot2/timechecked.txt', 'r')
    except IOError:
    ...
    if (now - datetime.datetime.strptime(timechecked[0],'%Y-%m-%d')).days >= 7 or newadd != []:
        newadd, newback = ping('testsample_weekly.csv', newadd, 30)
        backup += newback
    ...
    strnewtime += '\n'
    try:
        fh = open('/Users/ET/code/test_pingBot2/timechecked.txt', 'w')
    except IOError:
        print "Error: can\'t find file or read data"
        sys.exit()
    fh.write(strnewtime)
